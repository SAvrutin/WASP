...
int main()
{
    ...
    std::minstd_rand0 generator2 (SEED);
    ...
    //Sets up the output files///////////////////////////////////////////////////////////////////
      ...
      ofstream lambda_priors ("./RESULTS/lambda_priors_" + std::to_string(SEED) + ".txt", std::ios::out);
    ...
    ofstream annual_N_tot ("./RESULTS/N_ave_" + std::to_string(SEED) + ".txt", std::ios::out);
      ...
      //Sea Level Rise relative to 1986-2005 average (m) - not including land water storage///////
      ofstream annual_SeaLevel ("./RESULTS/SeaLevelRise.txt", std::ios::out);
      ofstream annual_SeaLevel_ice ("./RESULTS/SeaLevelRise_ice.txt", std::ios::out);
    ...
    //For making pdfs of the inputs////////////
    const int pdf_increments = 1000;

    double lambda_Planck_min = 0.0;
    double lambda_Planck_increment = 0.01;
    double lambda_WVLR_min = -10.0;
    double lambda_WVLR_increment = 0.1;
    double lambda_Cloud_SST_min = -30.0;
    double lambda_Cloud_SST_increment = 0.1;
    double ECS_total_min = 0.0;
    double ECS_total_increment = 0.05;
    double ECS_total = 0.0;
    double lambda_total_min = 0.0;
    double lambda_total_increment = 0.05;
    double lambda_total = 0.0;
    double lambda_normalised_min = -1.0;
    double lambda_normalised_increment = 0.01;
    double lambda_WVLR_normalised = 0.0;
    double lambda_Cloud_SST_normalised = 0.0;
    double AeroRF_total_2011 = 0.0;
    double AeroRF_total_2011_min = -5.0;
    double AeroRF_total_2011_increment = 0.01;
    double ice_coeff1_min = -0.005;
    double ice_coeff1_increment = 0.00001;
    double ice_coeff2_min = -0.005;
    double ice_coeff2_increment = 0.00001;
    double ice_melt_equil_min = 0;
    double ice_melt_equil_increment = 0.015;

    
    int lambda_Planck_count[pdf_increments];
    int lambda_WVLR_count[pdf_increments];
    int lambda_Cloud_SST_count[pdf_increments];
    int ECS_total_count[pdf_increments];
    int lambda_total_count[pdf_increments];
    int lambda_WVLR_normalised_count[pdf_increments];
    int lambda_Cloud_SST_normalised_count[pdf_increments];
    int AeroRF_total_2011_count[pdf_increments];
    int ice_coeff1_count[pdf_increments];
    int ice_coeff2_count[pdf_increments];
    int ice_melt_equil_count[pdf_increments];

    for(int n=0; n<pdf_increments; n++)
    {
        lambda_Planck_count[n]=0;
        lambda_WVLR_count[n]=0;
        lambda_Cloud_SST_count[n]=0;
        ECS_total_count[n]=0;
        lambda_total_count[n]=0;
        lambda_WVLR_normalised_count[n]=0;
        lambda_Cloud_SST_normalised_count[n]=0;
        AeroRF_total_2011_count[n] = 0;
        ice_coeff1_count[n] = 0;
        ice_coeff2_count[n] = 0;
        ice_melt_equil_count[n] = 0;
    }
    ////////////////////////////////////////////
    ...
    double cost_function_weighting = 1.0;
    double random_test_max = 1.0;
    ...
    //ice melt ones from Sandy
    double ice_coeff1; //= 0.003;
    double ice_coeff2; //= 0.002;
    double rate_ratio; // ratio of rate_10K to rate_1K
    //double rate_10K;
    double ice_melt_equil = 2.3;
    double LIG_dT = 2.0;
    double LIG_dZ = 7.0;
    ...
    //Indirect aerosol radiative forcing terms
    double beta_aci;
    double r1_aci;
    double r2_aci;
    double s_SOx_aci;
    double s_BCplusOC_aci;
    ...
    */
    ...
    std::vector<double> sea_level_rise; //To estimate total sea level rise (m)
    sea_level_rise.assign(tmax, 0.0);
    std::vector<double> Sea_surf_acid; //to estimate sea surface acidification (Delta pH units)
    Sea_surf_acid.assign(tmax, 0.0);


    std::vector<double> I_veg;          //terrestrial carbon stored in vegetation (PgC)
    ...
    for (int j = 0; j<SCENARIOS; j++) //This loop runs 'SCENARIOS' number of ensemble members
    {
        ...
        if(RESTORE == 1 )
        {
            ...
            R_WMGHG_nonCO2_2011 = R_WMGHG_nonCO2[(2011-Start_year)*t_step_per_year];
            R_aerosol_2011 = R_aerosol[(2011-Start_year)*t_step_per_year];
            ...
            if(Future_scenario<15 && Use_stored_params == 0)
            {
                ...
                a_CO2 = getRandomNormal2(5.35,0.27,generator(), generator(), generator.max(), generator.min());

                // SLR ice coefficients - Sandy Avrutin
                ice_coeff1 = getRandomUniform2(-0.005,0.005, generator(),  generator.max(), generator.min());

                //ice_coeff2 = getRandomUniform2(-0.002,0.002, generator(),  generator.max(), generator.min());
                //ice_coeff1 = getRandomUniform2(-0.5, 1.0, generator(),  generator.max(), generator.min());
                double rate_ratio = getRandomLogNormal2(log(1.0), log(2.0), generator(), generator(),  generator.max(), generator.min()); // assuming INSTANTANEOUS warming

                ice_coeff2 = (ice_coeff1*(1.0-rate_ratio))/(rate_ratio-10.0);
                //ice_coeff2 = 0.0;

                // cout << ice_coeff1 << "\t" << ice_coeff2 << "\t" << rate_ratio << "\t" << ice_melt_equil << endl;


                LIG_dT = getRandomUniform2(1.0, 2.0, generator(),  generator.max(), generator.min());
                LIG_dZ = getRandomNormal2(7.2, 1.3, generator(), generator(),  generator.max(), generator.min());

                ice_melt_equil = LIG_dZ/LIG_dT;

                for (int n=0; n<pdf_increments; n++)//100 points on each array
                {
                if(ice_coeff1 >= ice_coeff1_min+ice_coeff1_increment*double(n) && ice_coeff1 < ice_coeff1_min+ice_coeff1_increment*double(n+1) )
                    ice_coeff1_count[n] ++;
                if(ice_coeff2 >= ice_coeff2_min+ice_coeff2_increment*double(n) && ice_coeff2 < ice_coeff2_min+ice_coeff2_increment*double(n+1) )
                    ice_coeff2_count[n] ++;
                if(ice_melt_equil >= ice_melt_equil_min+ice_melt_equil_increment*double(n) && ice_melt_equil < ice_melt_equil_min+ice_melt_equil_increment*double(n+1) )
                    ice_melt_equil_count[n] ++;

                }


                //Use this function for palaeo prior distribution from standard error S distribution from Palaeosens
                //random_palaeosens1 = getRandomUniform2(0.0, 9.99022973240979E-01, generator(),  generator.max(), generator.min());
                ...
                dNPPdT =  getRandomUniform2(-5.0, 1.0, generator(),  generator.max(), generator.min());
                gamma_K = getRandomUniform2(0, 1.0, generator(),  generator.max(), generator.min());
                dtaudT =  getRandomUniform2(-1.36, 0.45, generator(),  generator.max(), generator.min());// mean +/- stdev of DGVMs in Pugh et al (2018)//getRandomUniform2(-2.0, 1.0, generator());
                ratio =    getRandomUniform2(0.2, 1.5, generator(), generator.max(), generator.min());//(0.25,1.1);
                ratio2 =  getRandomUniform2(0.1, 1.0, generator(),  generator.max(), generator.min());
                //epsilon = 1.0;//getRandomNormal(1.28,0.25);/////getRandomUniform2(0.83, 1.82, generator());///
                //epsilon_aero = 1.0;//getRandomUniform2(0.33, 3.0, generator());

                lambda_Planck = getRandomLogNormal2(log(3.3),sqrt(log(1.0+(0.1*0.1)/(3.3*3.3))), generator(), generator(), generator.max(), generator.min());// Planck feedback from 4*f_greenhouse*sigma*T^3// getRandomNormal(3.15,0.04);        //Planck feedback from CMIP5 models

                lambda_WVLR = getRandomLogNormal2(log(lambda_Planck),sqrt(log(2.0)), generator(), generator(), generator.max(), generator.min() ) - lambda_Planck; //getRandomUniform2(-3.0, 1.0, generator(),  generator.max(), generator.min()); //getRandomCauchy2(0.0, 3.3/12.5, generator()); //getRandomNormal2(lambda - lambda_Planck, (lambda-lambda_Planck)/1.0, generator() );//0.0;// getRandomNormal(-1.15,0.09); //getRandomUniform2(-3.15,2.0, generator(),  generator.max(), generator.min());//         //Combined Water Vapour - Lapse Rate


                lambda_Cloud_Fast = 0.0;// Use 0.0 when doing just one getRandomNormal(-0.43,0.33);   //Clouds with original SST patterns
                lambda_Cloud_SST = getRandomLogNormal2(log( lambda_Planck+lambda_WVLR ),sqrt(log(2.0)), generator(), generator(), generator.max(), generator.min() ) - (lambda_Planck+lambda_WVLR);//getRandomUniform2(-3.0, 3.0, generator()); //getRandomCauchy2(0.0, 3.3/12.5, generator()); //getRandomNormal(0.0,3.3);//getRandomUniform2(-2.0, 2.0, generator()); //getRandomNormal(-0.47,0.30);    //Clouds with final SST patterns


                lambda_albedo = 0.0;//getRandomNormal(0.0,3.3);//getRandomUniform2(-2.0, 2.0, generator()); //getRandomNormal(-0.37,0.10);      //getRandomUniform2(-3.15,2.0, generator());// Fast snow and ice-albedo
                ...
                //random_palaeosens1 = getRandomUniform2(0.0, 1.0027623323, generator());
                ...
                f_heat_ocean = 0.93;//getRandomUniform2(0.9, 0.96, generator(),  generator.max(), generator.min());

                tau_C_mixed = getRandomUniform2(0.5, 1.0, generator(),  generator.max(), generator.min()); //getRandomUniform2(0.1, 0.5, generator(),  generator.max(), generator.min());
                tau_C_upper = getRandomUniform2(5.0, 40.0, generator(),  generator.max(), generator.min());
                tau_C_inter = getRandomUniform2(15.0, 60.0, generator(),  generator.max(), generator.min());
                tau_C_deep =  getRandomUniform2(100.0, 500.0, generator(),  generator.max(), generator.min());
                tau_C_bottom = getRandomUniform2(400.0, 1500.0, generator(),  generator.max(), generator.min());
                Ib = getRandomUniform2(3100, 3900, generator(),  generator.max(), generator.min());

                //Set timescales from random distribution
                tau_WVLR = getRandomNormal2(0.024383562, 0.00109589, generator(), generator(), generator.max(), generator.min()); //8.9±0.4 days from residnece time of water vapour in Ent and Tuinenberg (2017)
                ...
                tau_albedo = getRandomUniform2(0.5,1.5, generator(),  generator.max(), generator.min());//respopnses on approx. annula timescales //getRandomUniform2(0.5,5.0, generator()); //half a year to 5 years: fast snow feedback to slow multi-year sea-ice feedback. Colman 2013 find multiple timescales from seasonal to decadal.
                tau_Cloud_SST = getRandomUniform2(20.0, 45.0, generator(),  generator.max(), generator.min()); //Andres uses first 20 years for fast - giving lower limit. Fine et al find 45 year timescale for tracers going through ventilated thermocline - giving upper limit.
                ...
                if(Future_scenario <=5)
                {
                    ...
                    R_aerosol_Uncert = getRandomNormal2(-0.057,0.4298, generator(), generator(), generator.max(), generator.min());//
                    ...
                    R_WMGHG_nonCO2_Uncert = getRandomNormal2(0.13,0.06079, generator(), generator(), generator.max(), generator.min());////getRandomNormal(0.32,0.06079); //
                }
                else
                {
                    R_aerosol_Uncert = 0.0;
                    R_WMGHG_nonCO2_Uncert = 0.0;
                }
                /*
                if(Future_scenario >=6 && Future_scenario <15)
                {
                   ...
                   // R_aerosol_Uncert = getRandomNormal(0.43026,0.6079);///getRandomNormal(0.074,0.6079);///
                    //Get to -0.35 + -0.45 Wm-2 with standard dev
                    R_aerosol_Uncert = getRandomNormal2(0.43026,0.6079, generator(), generator(), generator.max(), generator.min());///getRandomNormal(0.074,0.6079);///
                    R_WMGHG_nonCO2_Uncert = getRandomNormal2(-0.03,0.06079, generator(), generator(), generator.max(), generator.min());////getRandomNormal(0.32,0.06079); //


                }
                */
                //For uninformed distribution (-0.14 +/- 3 standard deviations)
                //R_aerosol_Uncert = getRandomUniform2(-1.9637,1.6837, generator(),  generator.max(), generator.min());
                ...
                R_volcanic_coeff = getRandomNormal2(-19.0, 0.5, generator(), generator(), generator.max(), generator.min());
                ...
                Uncert_CH4 = getRandomNormal2(1.0,0.07, generator(), generator(), generator.max(), generator.min());
                Uncert_N2O = getRandomNormal2(1.0,0.05, generator(), generator(), generator.max(), generator.min());
                //Uncert Halocarbons is from +/- 10% uncertainty in IPCC 2013 - where 10% is taken as 2stdevs
                Uncert_Halocarbons = getRandomNormal2(1.0,0.05, generator(), generator(), generator.max(), generator.min());
                ...
                gamma_aero_SOx = getRandomNormal2(-0.31,0.11, generator(), generator(), generator.max(), generator.min())/Aero_SOx_em[(2010-Start_year)];
                gamma_aero_BC = getRandomNormal2(0.18,0.07, generator(), generator(), generator.max(), generator.min())/Aero_BC_em[(2010-Start_year)];
                gamma_aero_OC = getRandomNormal2(-0.03,0.01, generator(), generator(), generator.max(), generator.min())/Aero_OC_em[(2010-Start_year)];
                gamma_aero_ENMVOC = getRandomNormal2(-0.06,0.09, generator(), generator(), generator.max(), generator.min())/Aero_ENMVOC_em[(2010-Start_year)];
                gamma_aero_NOx = 0.4*getRandomNormal2(-0.08,0.04, generator(), generator(), generator.max(), generator.min())/Aero_NOx_em[(2010-Start_year)];
                gamma_aero_NH3 = 0.6*getRandomNormal2(-0.08,0.04, generator(), generator(), generator.max(), generator.min())/Aero_NH3_em[(2010-Start_year)];


                //The aerosol-cloud interaction parameters. First the three independently assigned parameters
                AeroRF_ind_2011 = getRandomSkewNormnal2(-0.55, 0.37, -2.0, generator(), generator(), generator.max(), generator.min());

                beta_aci = getRandomLogNormal2(1.833, 3.353, generator(), generator(), generator.max(), generator.min());
                r1_aci = getRandomLogNormal2(-0.554,0.997,generator(), generator(), generator.max(), generator.min()); //ratio of s_BCplusOC to s_SOx

                //Now convert these three independently assigned parameters to calculate s_SOx and s_BCplusOC for aerosol cloud interaction
                r2_aci =  (r1_aci * Aero_SOx_em[(2011-Start_year)] + Aero_OC_em[(2011-Start_year)] + Aero_BC_em[(2011-Start_year)] ) / (-r1_aci*beta_aci*(exp(-AeroRF_ind_2011/beta_aci) - 1.0)) ;

                s_SOx_aci = -r2_aci*beta_aci;
                s_BCplusOC_aci = r1_aci * s_SOx_aci;
                ///////////////////////////////////////////////////////////////////////////////////



                ratio_volcanic_CO2=1.0;//getRandomUniform2(0.25,1.25,generator());
            ...
            if(Future_scenario>=15 || Use_stored_params == 1)
            {
                ...
                dtaudT =  dtaudT_obs[j];// mean +/- stdev of DGVMs in Pugh et al (2018)//getRandomUniform2(-2.0, 1.0, generator());
                ...
                tau_C_mixed = tau_C_mixed_obs[j]; //getRandomUniform2(0.1, 0.5, generator());
                ...
                beta_aci = beta_aci_obs[j];
                r1_aci = r1_aci_obs[j]; //ratio of s_BCplusOC to s_SOx
            ...
            //Put in code to make pdfs of input values here

            AeroRF_total_2011 = getR_aerosol(2011-Start_year, 0, gamma_aero_BC, gamma_aero_OC, gamma_aero_SOx, gamma_aero_NOx, gamma_aero_NH3, gamma_aero_ENMVOC, Start_year, AeroRF_ind_2011);

            for (int n=0; n<pdf_increments; n++)//100 points on each array
            {


               if(lambda_Planck >= lambda_Planck_min+lambda_Planck_increment*double(n) && lambda_Planck < lambda_Planck_min+lambda_Planck_increment*double(n+1) )
                    lambda_Planck_count[n] ++;

                if(lambda_WVLR >= lambda_WVLR_min+lambda_WVLR_increment*double(n) && lambda_WVLR < lambda_WVLR_min+lambda_WVLR_increment*double(n+1) )
                    lambda_WVLR_count[n] ++;

                if(lambda_Cloud_SST >= lambda_Cloud_SST_min+lambda_Cloud_SST_increment*double(n) && lambda_Cloud_SST < lambda_Cloud_SST_min+lambda_Cloud_SST_increment*double(n+1) )
                    lambda_Cloud_SST_count[n] ++;

                lambda_total = lambda_Planck + lambda_WVLR + lambda_Cloud_SST;

                if(lambda_total >= lambda_total_min+lambda_total_increment*double(n) && lambda_total < lambda_total_min+lambda_total_increment*double(n+1) )
                    lambda_total_count[n] ++;

                ECS_total = a_CO2 * log(2.0) / (lambda_Planck + lambda_WVLR + lambda_Cloud_SST);

                if(ECS_total >= ECS_total_min+ECS_total_increment*double(n) && ECS_total < ECS_total_min+ECS_total_increment*double(n+1) )
                    ECS_total_count[n] ++;

                lambda_Cloud_SST_normalised = ( lambda_Cloud_SST / (lambda_Planck+lambda_WVLR) );
                lambda_WVLR_normalised = (lambda_WVLR/lambda_Planck);

                if( lambda_Cloud_SST_normalised  >= lambda_normalised_min+lambda_normalised_increment*double(n) && lambda_Cloud_SST_normalised < lambda_normalised_min+lambda_normalised_increment*double(n+1) )
                   lambda_Cloud_SST_normalised_count[n] ++;

                if( lambda_WVLR_normalised  >= lambda_normalised_min+lambda_normalised_increment*double(n) && lambda_WVLR_normalised < lambda_normalised_min+lambda_normalised_increment*double(n+1) )
                    lambda_WVLR_normalised_count[n] ++;

                if( AeroRF_total_2011  >= AeroRF_total_2011_min+AeroRF_total_2011_increment*double(n) && AeroRF_total_2011 < AeroRF_total_2011_min+AeroRF_total_2011_increment*double(n+1) )
                    AeroRF_total_2011_count[n] ++;

            }

            if(lambda_WVLR < lambda_WVLR_min )
                lambda_WVLR_count[0] ++;
            if(lambda_WVLR > lambda_WVLR_min+lambda_WVLR_increment*double(pdf_increments) )
                lambda_WVLR_count[pdf_increments-1] ++;

            if(lambda_Cloud_SST < lambda_Cloud_SST_min )
                lambda_Cloud_SST_count[0] ++;
            if(lambda_Cloud_SST > lambda_Cloud_SST_min+lambda_Cloud_SST_increment*double(pdf_increments) )
                lambda_Cloud_SST_count[pdf_increments-1] ++;

            if(AeroRF_total_2011 < AeroRF_total_2011_min )
                AeroRF_total_2011_count[0] ++;
            if(AeroRF_total_2011 > AeroRF_total_2011_min+AeroRF_total_2011_increment*double(pdf_increments) )
                AeroRF_total_2011_count[pdf_increments-1] ++;
            ...
            //ice_coeff = getRandomUniform(0.0,0.005); //For T, Rahmstorf (2007) coefficient in units mm K-1 yr-1

            //land_water = getRandomUniform(-0.01,0.09);
        ...
        {
            //srand(j_obs[j]+1);
            //mt_rand.seed(j_obs[j]+1);
            generator2.seed(j_obs[j]+1);
        }
...

        {
            ...
            R_noise[i] = R_noise[i-1]*gamma_Rnoise + alpha_Rnoise*getRandomUniform2(-1.0, 1.0, generator2(),  generator.max(), generator.min());
            /*
            if(Future_scenario>= 15)
            {
                R_noise[i] = 0.0;
            ...
            */
            ...
            if(Future_scenario < 15)
            {
                ...
                //Single aerosol function, as used in Earth system dynamics paper in 2021
                ...
                //Separate aerosol direct and indirect aerosol radiative forcing, using three degrees of freedom on indirect aerosols
                //R_aerosol[i] = getR_aerosol_direct(year_count, t_step_count, gamma_aero_BC, gamma_aero_OC, gamma_aero_SOx, gamma_aero_NOx, gamma_aero_NH3, gamma_aero_ENMVOC) + getR_aerosol_indirect(year_count, t_step_count, s_BCplusOC_aci, s_SOx_aci, beta_aci);
           ...
           // cout << N_mixed[i] << endl;
            ...
            Heat_mixed[i] = Heat_mixed[i-1] + (f_heat_ocean*N_mixed[i] * area*60.0*60.0*24.0*365.0*dt) +
           ...
           //DeltaT[i] = (1.0/lambda_time[2][i])*(1.0 - (epsilon*N_mixed[i])/(a_CO2*log(CO2[i]/CO2init) + epsilon_aero*R_aerosol[i] + R_WMGHG_nonCO2[i] + R_solar[i] + epsilon_volcanic*R_volcanic[i] ) ) * ( (a_CO2/Ib)* (I_em[i] + I_Usat_mixed[i] + I_Usat_deep[i] + I_Usat_upper[i] + I_Usat_inter[i] + I_Usat_bottom[i] + I_eq_heat[i]) + epsilon_aero*R_aerosol[i] + R_WMGHG_nonCO2[i] + R_solar[i] + R_volcanic[i] ) + T_noise[i];
            ...
            //Sandy Avrutin T related ice melt SLR March 2021
            double rise = ice_melt_rise[i-1]+((ice_coeff1*DeltaT[i] + ice_coeff2*(DeltaT[i]*DeltaT[i]))*((ice_melt_equil*DeltaT[i]-ice_melt_rise[i-1])/(ice_melt_equil*DeltaT[i])))*dt;
            // make function asymmetrical
            if(rise >= ice_melt_rise[i-1])
            {
                ice_melt_rise[i] = rise;
            }

            else
            {
                ice_melt_rise[i] = ice_melt_rise[i-1];
            }
            ...
            if ( year[i] >= 2020.0-dt/2.0 && year[i] <= 2020.0+dt/2.0  )
            {
                ...
                */
                ...
                //If use sea level rise component, add in additional sea level observational constraints here (Sandy)

                //IPCC AR6 (Fox-Kemper et al 2021) has SLR FROM ICE of 1.03 with a 5-95% range of [0.76 to 1.35] (1971-2018) and 0.91 [0.61-1.21] (1901-1990)
                double SLRice_1993_2018_mmperyr = ((ice_melt_rise[((2018.0-Start_year)*t_step_per_year)+t_step_per_year-1]-ice_melt_rise[(1993.0-Start_year)*t_step_per_year]))*1e3/(2019.0-1993.0);
                double sig_ice_1993_2018 = (1.43-1.06)/(2*1.645);

                cost_function = cost_function*exp( -0.5*(((SLRice_1993_2018_mmperyr- 1.25)*(SLRice_1993_2018_mmperyr - 1.25))/(sig_ice_1993_2018*sig_ice_1993_2018)));

                double SLRice_1971_2018_mmperyr = ((ice_melt_rise[((2018.0-Start_year)*t_step_per_year)+t_step_per_year-1]-ice_melt_rise[(1971.0-Start_year)*t_step_per_year]))*1e3/(2019.0-1971.0);
                double sig_ice_1971_2018 = (1.35-0.76)/(2*1.645);


                cost_function = cost_function*exp( -0.5*(((SLRice_1971_2018_mmperyr- 1.03)*(SLRice_1971_2018_mmperyr - 1.03))/(sig_ice_1971_2018*sig_ice_1971_2018)));

                // terrestrial carbon check
                // mean of 127.05 PgC and a standard deviation of 32.625
                double Terrestrial_C_1959_2018 = 0.0;
                Terrestrial_C_1959_2018 = (I_ter[(2018-Start_year)*t_step_per_year] - I_ter[(1959-Start_year)*t_step_per_year]);

                cost_function = cost_function*exp( -0.5*(((Terrestrial_C_1959_2018- 127.05)*(Terrestrial_C_1959_2018 - 127.05))/(32.625*32.625)));

                double Terrestrial_C_2000_2009 = 0.0;
                Terrestrial_C_2000_2009 = (I_ter[((2009-Start_year)*t_step_per_year)+12.0] - I_ter[(2000-Start_year)*t_step_per_year])/10.0;

                cost_function = cost_function*exp( -0.5*(((Terrestrial_C_2000_2009- 2.6)*(Terrestrial_C_2000_2009 - 2.6))/(0.73*0.73)));
                ...
                {
                    random_test_max = 0.35;
                    random_test = getRandomUniform2(0.0,random_test_max, generator(),  generator.max(), generator.min());
                }
                if(T_data == 1 && OHC_data == 0)//Cowtan&Way + Cheng et al.
                {
                    random_test_max = 0.025;
                    random_test = getRandomUniform2(0.0,random_test_max, generator(),  generator.max(), generator.min());
                }
                ...
                if(T_data == 1 && OHC_data == 1)//Cowtan&Way + NODC SANDY - THIS ONE
                {
                    random_test_max = 0.0025;
                    random_test = getRandomUniform2(0.0,random_test_max, generator(),  generator.max(), generator.min());
                }

                if(T_data == 2 && OHC_data == 0)//HadCRUT5 + Cheng et al.
                {
                    random_test_max = 1.0e-9;
                    random_test = getRandomUniform2(0.0,random_test_max, generator(),  generator.max(), generator.min());
                }
                if(T_data == 3 && OHC_data == 0)//HadCRUT5 (no infilling) + Cheng et al.
                {
                    random_test_max = 0.09;
                    random_test = getRandomUniform2(0.0,random_test_max, generator(),  generator.max(), generator.min());
                ...
                {
                    data_fit = 12;
                    if(cost_function > random_test_max)
                        cost_function_weighting = cost_function/random_test_max;
                    if(cost_function <= random_test_max)
                        cost_function_weighting = 1.0;
                }
               ...
               */
               ...
               //If use sea level rise component, add in additional sea level observational constraints here
                ...
                if(data_fit >= 10)
                {
                    ...
                    inputs << j << '\t' << ice_coeff1<< '\t' << ice_coeff2 << '\t' << cost_function_weighting << '\t' << ice_melt_equil << '\t' << lambda << '\t' <<  lambda_Planck << '\t' <<  lambda_WVLR << '\t' << lambda_Cloud_Fast << '\t' << lambda_albedo << '\t' << lambda_Cloud_SST << '\t' << lambda_1000yr << '\t' << tau_WVLR << '\t' << tau_Cloud_Fast << '\t' << tau_albedo << '\t' <<  tau_Cloud_SST << '\t' << ratio << '\t' << ratio2 << '\t' << tau_C_mixed << '\t' << tau_C_upper << '\t' << tau_C_inter <<'\t' << tau_C_deep << '\t' << tau_C_bottom << '\t' << Ib << '\t' << a_CO2 << '\t' << R_aerosol_Uncert << '\t' << R_WMGHG_nonCO2_Uncert << '\t' << R_volcanic_coeff <<  '\t' << f_heat_ocean << '\t' << dNPPdT << '\t' << dtaudT << '\t' << gamma_K << '\t' << ratio_volcanic_CO2 << '\t' << gamma_aero_SOx << '\t' << gamma_aero_NOx << '\t' << gamma_aero_OC << '\t' << gamma_aero_BC << '\t' << gamma_aero_NH3 << '\t' << gamma_aero_ENMVOC << '\t' << AeroRF_ind_2011  << '\t' << beta_aci << '\t' << r1_aci << '\t' << Uncert_CH4 << '\t' << Uncert_N2O << '\t' << Uncert_Halocarbons << '\t' << AeroRF_total_2011 << endl;
                    ...
                    if(Future_scenario <15 && Use_stored_params == 0)
                    {
                        ...
                        beta_aci_obs.push_back(beta_aci);
                        r1_aci_obs.push_back(r1_aci);
                        j_obs.push_back(j);
                    }
            ...
            if (/*output == t_step_per_year-1 &&*/ year[i] >= 2301.0-dt/2.0 && year[i] <= 2301+dt/2.0 )
            {
                ...
                for (int n=0; n<20*t_step_per_year; n++)//Add up contributions
                {
                    ...
                    DS_86_05_ice += (ice_melt_rise[i-(315*t_step_per_year-n)])/(20.0*double(t_step_per_year));
                ...
                if(data_fit>=10)
                {
                    ...
                    int years_back = 451;// year[i] - Start_year - 1; //535;
                    if(Future_scenario >= 15)
                    {
                    ...
                    //Output stored properties by year
                        for (int i2 = -years_back; i2 <=0; i2 ++)
                        {
                            ...
                            //NPP_flux << NPP_ave << '\t' ;
                            annual_SeaLevel << ice_melt_rise[i+i2*t_step_per_year] + Steric_rise[i+i2*t_step_per_year] - DS_86_05 << '\t';
                            annual_SeaLevel_ice << ice_melt_rise[i+i2*t_step_per_year] - DS_86_05_ice <<'\t';
                            ...
                            if(i2 == 0)
                            {
                                ...
                                annual_SeaLevel << endl;
                                annual_SeaLevel_ice << endl;
    ...
    //Output the pdf increments
    for (int n = 0; n<pdf_increments; n++)
    {
        lambda_priors << ice_coeff1_min + ice_coeff1_increment*(double(n) + 0.5) << '\t' << ice_coeff1_count[n] << '\t' << ice_coeff2_min + ice_coeff2_increment*(double(n) + 0.5) << '\t' << ice_coeff2_count[n] << '\t' << ice_melt_equil_min + ice_melt_equil_increment*(double(n) + 0.5) << '\t' << ice_melt_equil_count[n] << '\t' << lambda_Planck_min + lambda_Planck_increment*(double(n) + 0.5) << '\t' << lambda_Planck_count[n] << '\t' << lambda_WVLR_min + lambda_WVLR_increment*(double(n) + 0.5) << '\t' << lambda_WVLR_count[n] << '\t' << lambda_Cloud_SST_min + lambda_Cloud_SST_increment*(double(n) + 0.5) << '\t' << lambda_Cloud_SST_count[n] << '\t'  << lambda_total_min + lambda_total_increment*(double(n) + 0.5) << '\t' <<  lambda_total_count[n] <<'\t'  << ECS_total_min + ECS_total_increment*(double(n) + 0.5) << '\t' <<  ECS_total_count[n] << '\t' << lambda_normalised_min + lambda_normalised_increment*(double(n) + 0.5) << '\t' << lambda_WVLR_normalised_count[n] << '\t' << lambda_Cloud_SST_normalised_count[n] << '\t' << AeroRF_total_2011_min + AeroRF_total_2011_increment*(double(n)+0.5) << '\t' << AeroRF_total_2011_count[n] << endl;



    }
    // cout << ice_coeff1 << "\t" << ice_coeff2 << endl;


    //If there have been any observation-consistent simulations then output the results
    if(a_CO2_obs.size()>=1)
    {
        stored_inputs << std::hexfloat << std::showbase  ;
        ...
        stored_inputs << "beta_aci_obs={";
        for (int i = 0; i<count_obs-1; i++)
        {
            stored_inputs << beta_aci_obs[i] << "," ;
        }
        stored_inputs << beta_aci_obs[count_obs-1] << "};" << endl;

        stored_inputs << "r1_aci_obs={";
        for (int i = 0; i<count_obs-1; i++)
        {
            stored_inputs << r1_aci_obs[i] << "," ;
        }
        stored_inputs << r1_aci_obs[count_obs-1] << "};" << endl;
    ...
    annual_warming_blended.close();
    annual_warming_SST.close();
    annual_lambda.close();
